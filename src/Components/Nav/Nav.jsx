...
function Nav() {
  ...
  return (
    <div className='z-50 py-3 sm:px-20 px-3 flex items-center justify-between'>
     ...
     <ul className='font-medium menu transition-all z-[99] text-zinc-800/70 flex top-0 w-3/4 sm:w-auto flex-col sm:flex-row items-center gap-3 uppercase fixed sm:relative sm:right-auto -right-full sm:pt-0 pt-24 h-full sm:bg-transparent bg-zinc-200'>
          <li><NavLink to='/' className={({isActive})=>`${ isActive ? 'text-orange-400' : 'text-zinc-800/80'} hover:text-orange-600`} >Today's price</NavLink></li>
          <li><NavLink to='Collection' className={({isActive})=>`${ isActive ? 'text-orange-400' : 'text-zinc-800/80 hover:text-orange-600'}`} >Collection</NavLink></li>
          <li><NavLink to='Gold' className={({isActive})=>`${ isActive ? 'text-orange-400' : 'text-zinc-800/80'} hover:text-orange-600`} >Gold</NavLink></li>
